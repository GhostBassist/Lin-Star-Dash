...
<?php if (!empty($_POST)):
	...
	if (empty($_POST['factoryReset']) != TRUE ) {
	  ...
	  exec('sudo git --work-tree=/usr/local/sbin --git-dir=/usr/local/sbin/.git reset --hard origin/main');
	  exec('sudo git --work-tree=/usr/local/bin --git-dir=/usr/local/bin/.git reset --hard origin/main');
    exec('sudo chmod +x /usr/local/sbin/*');
    exec('sudo chmod +x /usr/local/bin/*');
    //TEMP DISABLED DASH UPDATE - EC 2022
	  //exec('sudo git --work-tree=/var/www/dashboard --git-dir=/var/www/dashboard/.git reset --hard origin/main');
          echo '<script type="text/javascript">setTimeout(function() { window.location=window.location;},5000);</script>';
